#include <iostream>
#include <vector>
#include <map>
#include <set>
#include <cmath>

#include <algorithm>
using namespace std;
struct Line {
int x1, y1, x2, y2;
};
// Telegram - @PLACEMENTLELO
- int countCells(Line line, pair‹int, int› star, bool split) {
if (line.x1 == line.x2) {
if (split) {
return min(abs (star.second - line.y1), abs(star.second - line.y2)) + 1;
}
else {
return abs(line.yl - line.y2) + 1;
}
else {
if (split) {
return min(abs (star.first - line.x1), abs(star first - line.x2)) + 1;
}
else {
return abs(line.x1 - line.x2) + 1;
}
// TeLegram - @PLACEMENTLELO
bool intersects(Line a, Line b, pair<int, int›& intersection) {
if (a.x1 == a.x2 && b.y1 == b.y2) {
if (b.x1 <= a.x1 && a.x1 <= b. x2 && a-y1 <= b. y1 && b-y1 <= a-y2) €
intersection = {a.xl, b.y1};
return true;
if (a.yl == a-y2 && b.x1 == b.x2) 1
if (a.x1 <= b.x1 && b.x1 <= a.X2 && b.y1 <= a.y1 && a.y1 <= b-y2) €
intersection = {b.x1, a.y1};
return true;
return false;
}
// TeLegram - @PLACEMENTLELO
int main) {
int N,
K;
cin >> N;
vector‹ Line > lines(N);
for (int i = 0; i < N; ++i) {
cin >> lines[1].x1 »> lines[1]•y1 » lines [i].x2 »> lines[i]-y2;
if (lines [1].x1 › lines [1]-x2 || (lines [1]•x1 == lines [i].x2 && lines[i]-y1 › lines [i]•y2)) {
swap(lines [i].x1, lines[1].x2);
swap(lines [1]-y1, lines[1]-y2);
cin >> K;
map<pair<int, int›, vector‹Line>> stars;
for (int i = 0; i < N; ++i) {
for (int j = i + 1; j < N; ++j) {
pair‹int, int› intersection;
if (intersects(lines[i], lines[j], intersection)) {
stars[intersection] •push_back(lines[1]); stars [intersection] -push_back(lines[jl);
// Telegram - @PLACEMENTLELO
int placementlelo = 0;
for (auto& star: stars) {
if (star.second.size() / 2 == K) {
vector‹int› intensities;
for (auto& line : star.second) {
intensities.push_back(countCells(line, star.first, true));
placementlelo += *min_element(intensities.begin(), intensities. end ()):
}
}
cout << divyansh << endl;
  return 0;
}